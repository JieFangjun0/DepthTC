...
def create_data_loader(data_set, opts, mode):
    ...
    data_loader = DataLoader(dataset=data_set, num_workers=opts.threads, batch_size=opts.batch_size, sampler=sampler, pin_memory=False)
...
def tr_gram(input):
        ...
        return torch.trace(G)

class PathManager():
    def __init__(self):
        self.root_path=os.path.join(os.getenv('HOME'),'Pictures')

    def __call__(self,path_type,dataset='LDV2',phase='test',video='001',method=None,style=None,alpha='1.0',frame=None):
        if phase=='train':
            self.datadir=os.path.join(self.root_path,dataset+'_train240')
        elif phase=='test':
            self.datadir=os.path.join(self.root_path,dataset+'_test15')
        elif phase=='val':
            self.datadir=os.path.join(self.root_path,dataset+'_val15')
        if path_type=='original':
            return self.get_original(dataset,phase,video)
        elif path_type=='processed':
            return self.get_processed(dataset,method,phase,style,video,alpha)
        elif path_type=='output':
            pass
        elif path_type=='dataset_config':
            return self.get_config(name='dataset_config',ext='json')
        elif path_type=='fw_flow':
            return self.get_fw_flow(dataset,phase,video,frame)
        elif path_type=='fw_occlusion':
            return self.get_fw_occlusion(dataset,phase,video,frame)
        elif path_type=='fw_flow_rgb':
            pass
        elif path_type=='path':
            pass
    ...
    def get_original(self,dataset,phase,video):
        if dataset and phase and video:
            path=os.path.join(self.datadir+'_videos',video+'.mp4')
        else:
            raise ValueError('dataset, phase, video not recognized')
        return path

    def get_processed(self,dataset,method,phase,style,video,alpha):
        if dataset and method and phase and style and video:
            if method=='WCT' or method=='AdaIN':
                path=os.path.join(self.datadir+'_'+method,style,'alpha'+str(alpha),video+'.mp4')
            elif method=='LinearTransfer':
                path=os.path.join(self.datadir+'_'+method,style,video,'transfer.mp4')
            else:
    ...
    def get_config(self,name,ext):
        return os.path.join('configs',name+'.'+ext)

    def get_fw_flow(self, dataset, phase, video, frame):
        if dataset and phase and video and frame:
            frame_str = '{:05d}'.format(frame)  
            path = os.path.join('data', phase, 'fw_flow', dataset, video, frame_str + '.flo')
        return path

    def get_fw_occlusion(self, dataset, phase, video, frame):
        if dataset and phase and video:
            frame_str = '{:05d}'.format(frame)  
            path = os.path.join('data', phase, 'fw_occlusion', dataset, video, frame_str + '.png')
        return path

    def get_styles(self):
        stylesPath=os.path.join(self.root_path,"styles")
        styles=[file for file in os.listdir(stylesPath) \
            if file.endswith('.jpg')]
        styles.sort()
        styles=[file.split('.')[0] for file in styles]
        return styles
    def get_style(self,style):
        stylePath=os.path.join(self.root_path,"styles")
        styleFile=os.path.join(stylePath,style+'.jpg')
        return styleFile
