...
backwarp_tenGrid = {}    #放缩至（-1，1）之后的坐标层
backwarp_tenPartial = {} #与图片等大小，值为1
...
##
class Resample2d(torch.nn.Module): #与TC代码耦合
    def __init__(self):
        super().__init__()
    ...
    def forward(self,tenInput, tenFlow):
        return self.backwarp(tenInput,tenFlow)
    
class Network(torch.nn.Module): #与TC代码耦合
    def __init__(self):
        super().__init__()
...
##########################################################
class Flownet(torch.nn.Module): #与TC代码耦合
...
def estimate(tenOne, tenTwo,device="cuda"):
    global netNetwork
