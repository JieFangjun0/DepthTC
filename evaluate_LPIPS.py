...
if __name__ == "__main__":
    ...
    parser.add_argument('-net',             type=str,     default="alex",        choices=["alex", "vgg", "squeeze"], help='LPIPS model')
    parser.add_argument('-redo',            action="store_true",                    help='redo evaluation')

    opts = parser.parse_args()
    print(opts)

    output_dir = os.path.join(opts.data_dir, opts.phase,"output" ,opts.method, opts.task, opts.dataset)
    ...
    import lpips
    model = lpips.LPIPS(net=opts.net)
    ...
    for v in range(len(video_list)):
        ...
        output_dir = os.path.join(opts.data_dir, opts.phase, "output",opts.method, opts.task, opts.dataset, video)
