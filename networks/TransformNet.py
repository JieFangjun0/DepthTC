...
class TransformNet(nn.Module):
    ...
    def __init__(self, opts, nc_in, nc_out):
        ...
        ## convolution layers
        self.conv1a = ConvLayer(1, nf * 1, kernel_size=7, stride=1, bias=use_bias, norm=opts.norm) ## input: O1
        self.conv1b = ConvLayer(1, nf * 1, kernel_size=7, stride=1, bias=use_bias, norm=opts.norm) ## input: D2
    ...
    def forward(self, X, prev_state):
        # parm=[pm.requires_grad for pm in self.conv1a.parameters()]
        # logger.debug(parm)
        Xa = X[:,:1,:,:] ## O1
        Xb = X[:,1:,:,:] ## D2
